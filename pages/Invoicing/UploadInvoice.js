    ...
    <div>
      {!loading && (
        <div>
          ...
          <p className="mt-2 text-2xl text-center font-bold mb-4">INVOICE</p>
          <p className="mt-8 text-xl mb-4 text-center font-bold">
            Upload Invoice
          </p>
          {clientRequests.map((clientRequest) => (
            <div
              key={clientRequest.id}
              className="items-center sm:mx-24 grid grid-cols-3 place-items-center gap-x-12 mb-4"
            >
              <p className="text-lg">
                {clientRequest.name} (<span>{clientRequest.clientId}</span>)
              </p>
              {/* <div>
                {isCameraOpen ? (
                  <div>
              ...
              <button
                className={` bg-gray-400 p-2 px-4 font-semibold`}
                onClick={() => {
                  const params = new URLSearchParams({
                    uploadInvoice: true,
                    address: clientRequest.address,
                    aspects: clientRequest.aspects,
                    clientCode: clientRequest.clientCode,
                    clientId: clientRequest.clientId,
                    ...
                    showroom: clientRequest.showroom,
                    sourceInfo: clientRequest.sourceInfo,
                    specificInfo: clientRequest.specificInfo,
                  });
                  const queryString = `/erpag/SalesOrder?${params.toString()}`;
              ...
              {/* <button
                className={`${
                  uploadStatus[clientRequest.id] ? `bg-green-400` : `bg-red-400`
                }  p-2`}
