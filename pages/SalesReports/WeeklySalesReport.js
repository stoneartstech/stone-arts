...
export default function WeeklySalesReport() {
    ...
    function handleExportToExcel(report) {
        const ws_data = [
            ["S. No.", "Date", "Client Name", "Supply", "Supply & Fix", "Invoice No.", "Invoice Value", "Tax Amount", "Total including Tax", "Payment", "Mode of Payment", "Balance"],
            ...Object.keys(report).map((key, index) => [
                index + 1,
                report[key].Date,
                report[key].ClientName,
                report[key].Supply,
                report[key].SupplyFix,
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Weekly Sales Report');
        ...
        const fileName = `Weekly_Sales_Report_${report[0].Date}_${showroomName}.xlsx`;
            ...
            {!loading && (
                <div>
                    ...
                    <div className='flex flex-col items-center'>
                        <p className='text-3xl mb-4'>Weekly Sales Reports History</p>
                    </div>
                                        ...
                                        <tbody>
                                            {Object.keys(report).map((key, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td className='border border-black p-2'>{report[key].Supply}</td>
                                                    <td className='border border-black p-2'>{report[key].SupplyFix}</td>
