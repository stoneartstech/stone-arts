...
export default function PendingClientDesigns() {
  ...
  async function handleClientApproval(design) {
    const designData = designs.find((item) => item.id === design?.id);
    await setDoc(
      doc(db, "designer" + design?.designerId + "-completed", design?.id),
      designData
    );
    await deleteDoc(doc(db, dbName, design?.id));
    alert(`Project ${design?.id} got approved by Client`);
  }
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center gap-12 my-4">
        <p className="my-4 text-3xl text-center">
          Designs Pending Client Approval
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4 mt-8 items-center">
        {designs.map((design) => (
          ...
          >
            ...
            <button
              onClick={() => handleClientApproval(design)} // Attach onClick event here
              className="bg-green-400 p-2 rounded-lg text-center"
            >
              Client Approved
            </button>
