...
export default function SupervisorWeeklyReportUpload() {
  ...
  const handleUploadReportImages = async (qsId, event) => {
    ...
    try {
      ...
      const storageRef = ref(
        storage,
        `PMTImages/SupervisorWeekly/${date}/${qsId}`
      );
      ...
      <div className="max-w-full py-4 border border-black px-2 overflow-auto">
        <div>
          <div className=" flex items-center justify-between">
            <div className=" flex flex-col gap-2 md:flex-row w-full items-center justify-between ">
              ...
              <div className=" border border-black font-semibold px-4 py-0.5 w-fit p-0.5 ">
                <p className=" py-1.5">
                  Week : {weekStart}/{month}/{year} - {weekEnd}/{month}/{year}
                </p>
              </div>
            </div>
          </div>
        ...
        <button
          ...
          onClick={() => {
            ...
            setDoc(doc(db, `PMTReports/SupervisorReports/Weekly/${date}`), {
